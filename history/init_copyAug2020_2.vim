...
function Run_Super_OpenIO()
    ...
    "after confirming we are on main file, close all others
    :wincmd t
    :silent only

    let l:ipexists = bufexists(bufname("input.txt"))
    let l:opexists = bufexists(bufname('output.txt'))
    let l:ipnr2=bufwinnr(bufname('input.txt')) 
    let l:opnr2=bufwinnr(bufname('output.txt'))

    if (l:ipexists==0 && l:opexists==0)||(l:ipnr2==-1 && l:opnr2==-1)
    ...
    let l:newipid = bufwinid(bufname('input.txt'))
