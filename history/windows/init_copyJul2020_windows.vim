...
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"Startify stuff
let g:startify_bookmarks = ['~/AppData/Local/nvim/init.vim']  "A list of files or directories to bookmark.
...
function Debug_soln()
    ...
    :call jobsend(b:terminal_job_id, "gdb .\a.exe -tui\n")
    :exe "normal a" 
...
function Run_soln(code)
    ...
    if a:code == 1
        ":! .\a.exe
        :silent ! .\a.exe < input.txt > output.txt & sleep 2s && kill $(jobs -p)
    elseif a:code == 2
        :belowright split | terminal
        :call jobsend(b:terminal_job_id, ".\a.exe \n")
        :exe "normal a" 
    elseif a:code == 3
        :silent ! .\a.exe < input.txt > output.txt & sleep 0.2s && kill $(jobs -p)
        "Test this once
    else
        :silent ! .\a.exe < ginput.txt > output.txt & sleep 1s && kill $(jobs -p)
    endif

    "You can even route some other program's output to .\a.exe, like
    ".\a.exe < ./m.out > output.txt
endfunction

"working fine
function Run_generator()
    :wa
    :! g++ -O2 -Wall -Wextra -pedantic-errors -std=c++17 generator.cpp -o generator.exe
    :silent ! ./generator.exe > ginput.txt & sleep 1s && kill $(jobs -p)
endfunction
...
function Run_csoln(code)
    :wa
    if a:code == 1
        :! g++ -O2 -Wall -Wextra -pedantic-errors -std=c++17 csoln.cpp -o csoln.exe
        :silent ! ./csoln.exe < ginput.txt > coutput.txt & sleep 1s && kill $(jobs -p)
    else
        :! g++ -O2 -Wall -Wextra -pedantic-errors -std=c++17 csoln.cpp -o csoln.exe
        :silent ! ./csoln.exe < input.txt > coutput.txt & sleep 1s && kill $(jobs -p)
    endif
endfunction
...
function Run_checker(code)
    :wa
    if a:code == 1
        :! g++ -O2 -Wall -Wextra -pedantic-errors -std=c++17 checker.cpp -o checker.exe
        :silent ! ./checker.exe ginput.txt > result.txt & sleep 1s && kill $(jobs -p)
    else
        :! g++ -O2 -Wall -Wextra -pedantic-errors -std=c++17 checker.cpp -o checker.exe
        :silent ! ./checker.exe input.txt > result.txt & sleep 1s && kill $(jobs -p)
    endif
endfunction
...
function Pull_SpecialQuestions_Cpp()
    "pull at cursor
    ":0  "Not deleting anything in this command to avoid accidental deletion
    :r E:\Documents\cp\nglib\special\SPECIAL_ALGOS.cpp
endfunction
